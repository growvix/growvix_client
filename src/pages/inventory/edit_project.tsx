...
const initialFormData: ProjectFormData = {
    ...
    location: '',
    preferred: [],
    img_location: { logo: '', banner: '', brochure: '', post: '', videos: '' },
    blocks: [],
    plots: [],
...
const generateUnitsFromTypes = (floorNumber: number, unitTypes: UnitTypeConfig[], pattern: string): Unit[] => {
    ...
    unitTypes.forEach(unitType => {
        for (let i = 0; i < unitType.count; i++) {
            let unitNumber = ''
            if (pattern === 'numeric') unitNumber = `${floorNumber}${String(unitIndex).padStart(2, '0')}`
            else if (pattern === 'alpha') unitNumber = `${floorNumber}${String.fromCharCode(64 + unitIndex)}`
            else unitNumber = `${floorNumber}-${unitIndex}`
            units.push({
                unitId: unitNumber, unitNumber, bhk: unitType.bhk, bathrooms: unitType.bathrooms,
                size: unitType.size, unitType: unitType.name, facing: 'East', status: 'available',
...
interface BlockConfig { floors: number; pattern: string; unitTypes: UnitTypeConfig[] }

export default function EditProject() {
    const { id } = useParams()
    const navigate = useNavigate()
    const decodedId = id ? (decodeProjectId(id) || "") : ""
    ...
    const updatePlot = (plotId: string, updates: Partial<Plot>) => {
        setFormData(prev => ({ ...prev, plots: prev.plots?.map(p => p.plotId === plotId ? { ...p, ...updates } : p) }))
    }
    ...
    const generatePlots = () => {
        ...
        for (let i = 0; i < plotsConfig.totalPlots; i++) {
            ...
            plots.push({ plotId: `plot-${plotNum}`, plotNumber: plotNum, size: plotsConfig.defaultSize, facing: 'East', status: 'available' })
        }
    ...
    const getBlockConfig = (blockId: string): BlockConfig => {
        return blockConfigs[blockId] || { floors: 10, pattern: 'numeric', unitTypes: [{ name: 'Type A', bhk: 2, bathrooms: 2, size: 1200, count: 4 }] }
    }

    const updateBlockConfig = (blockId: string, updates: Partial<BlockConfig>) => {
        setBlockConfigs(prev => ({ ...prev, [blockId]: { ...getBlockConfig(blockId), ...updates } }))
    }
    ...
    const addUnitType = (blockId: string) => {
        ...
        updateBlockConfig(blockId, { unitTypes: [...config.unitTypes, { name: newTypeName, bhk: 2, bathrooms: 2, size: 1200, count: 2 }] })
    }
    ...
    const removeUnitType = (blockId: string, index: number) => {
        ...
        const resequencedTypes = filteredTypes.map((type, i) => ({ ...type, name: `Type ${String.fromCharCode(65 + i)}` }))
    ...
    useEffect(() => {
        ...
        const fetchProject = async () => {
            try {
                ...
                const res = await axios.get(API.getProject(decodedId), {
                ...
                setFormData({
                    name: project.name || '',
                    property: project.property || 'apartments',
                    location: project.location || '',
                    preferred: project.preferred || [],
                    img_location: project.img_location || { logo: '', banner: '', brochure: '', post: '', videos: '' },
                    blocks: project.blocks || [],
                    plots: project.plots || [],
                    plotNumberPattern: project.plotNumberPattern || 'numeric',
                    layoutImages: project.layoutImages || []
                })

                // Reconstruct blockConfigs from existing blocks
                if (project.blocks && project.blocks.length > 0) {
                    const configs: Record<string, BlockConfig> = {}
                    project.blocks.forEach((block: Block) => {
                        const unitTypesMap = new Map<string, UnitTypeConfig>()
                        if (block.floors && block.floors.length > 0) {
                            const firstFloor = block.floors[0]
                            firstFloor.units.forEach((unit: Unit) => {
                                const key = `${unit.bhk}-${unit.bathrooms}-${unit.size}`
                                if (!unitTypesMap.has(key)) {
                                    unitTypesMap.set(key, { name: unit.unitType || `${unit.bhk}BHK`, bhk: unit.bhk, bathrooms: unit.bathrooms, size: unit.size, count: 0 })
                                }
                                unitTypesMap.get(key)!.count++
                            })
                        }
                        configs[block.blockId] = {
                            floors: block.totalFloors || block.floors?.length || 10,
                            pattern: 'numeric',
                            unitTypes: unitTypesMap.size > 0 ? Array.from(unitTypesMap.values()) : [{ name: 'Type A', bhk: 2, bathrooms: 2, size: 1200, count: 4 }]
                        }
                    })
                    setBlockConfigs(configs)
                    setExpandedBlocks(project.blocks.map((b: Block) => b.blockId))
                }

                // Reconstruct plotsConfig from existing plots
                if (project.plots && project.plots.length > 0) {
                    setPlotsConfig({ totalPlots: project.plots.length, defaultSize: project.plots[0].size || 1200 })
                }
            } catch (err: any) {
    ...
    const addBlock = () => {
        const blockId = String.fromCharCode(65 + formData.blocks.length)
        const newBlock: Block = { blockId, blockName: `Block ${blockId}`, totalFloors: 5, floorPlanImages: [], floors: [] }
    ...
    const removeBlock = (blockIdToRemove: string) => {
        setFormData(prev => {
            ...
            const reindexedBlocks = filteredBlocks.map((block, index) => {
                const newBlockId = String.fromCharCode(65 + index)
                return { ...block, blockId: newBlockId, blockName: `Block ${newBlockId}` }
            })
        ...
        setExpandedBlocks(prev => {
            ...
            return prev.filter(id => id !== blockIdToRemove).map(id => {
                const oldIndex = formData.blocks.findIndex(b => b.blockId === id)
                if (oldIndex > index) return String.fromCharCode(65 + oldIndex - 1)
                return id
            })
        })
    }
    ...
    const updateBlock = (blockId: string, field: string, value: any) => {
        setFormData(prev => ({ ...prev, blocks: prev.blocks.map(b => b.blockId === blockId ? { ...b, [field]: value } : b) }))
    }
    ...
    const generateFloors = (blockId: string) => {
        ...
        for (let i = 1; i <= config.floors; i++) {
            floors.push({ floorNumber: i, floorName: getFloorName(i), units: generateUnitsFromTypes(i, config.unitTypes, config.pattern) })
        }
    ...
    const copyFloorToOthers = (sourceBlockId: string, sourceFloorNum: number, targetFloorNums: number[]) => {
        ...
        sourceFloor.units.forEach(unit => {
            const key = `${unit.bhk}-${unit.bathrooms}-${unit.size}`
            if (!unitTypesMap.has(key)) unitTypesMap.set(key, { name: unit.unitType || `${unit.bhk}BHK`, bhk: unit.bhk, bathrooms: unit.bathrooms, size: unit.size, count: 0 })
            unitTypesMap.get(key)!.count++
        })
    ...
    const updateSingleUnit = (blockId: string, floorNumber: number, unitId: string, updates: Partial<Unit>) => {
        setFormData(prev => ({
            ...prev, blocks: prev.blocks.map(b => {
                ...
                return { ...b, floors: b.floors.map(f => { if (f.floorNumber !== floorNumber) return f; return { ...f, units: f.units.map(u => u.unitId !== unitId ? u : { ...u, ...updates }) } }) }
            })
        }))
    }
    ...
    const addUnitToFloor = (blockId: string, floorNumber: number) => {
        ...
        let unitNumber = ''
        if (config.pattern === 'numeric') unitNumber = `${floorNumber}${String(nextNum).padStart(2, '0')}`
        else if (config.pattern === 'alpha') unitNumber = `${floorNumber}${String.fromCharCode(64 + nextNum)}`
        else unitNumber = `${floorNumber}-${nextNum}`
        const newUnit: Unit = { unitId: unitNumber, unitNumber, bhk: 2, bathrooms: 2, size: 1200, unitType: 'Type A', facing: 'East', status: 'available', position: { row: Math.floor((nextNum - 1) / 4), col: (nextNum - 1) % 4 } }
        setFormData(prev => ({
            ...prev, blocks: prev.blocks.map(b => {
                if (b.blockId !== blockId) return b
                return { ...b, floors: b.floors.map(f => { if (f.floorNumber !== floorNumber) return f; return { ...f, units: [...f.units, newUnit] } }) }
            })
        }))
    }
    ...
    const removeUnitFromFloor = (blockId: string, floorNumber: number, unitId: string) => {
        setFormData(prev => ({
            ...prev, blocks: prev.blocks.map(b => {
                if (b.blockId !== blockId) return b
                return { ...b, floors: b.floors.map(f => { if (f.floorNumber !== floorNumber) return f; return { ...f, units: f.units.filter(u => u.unitId !== unitId) } }) }
            })
        }))
    }
    ...
    const handleLayoutImageUpload = async (files: FileList | null) => {
        ...
        try {
            const response = await axios.post(API.UPLOAD.FLOOR_PLANS, formDataUpload, { headers: { 'Content-Type': 'multipart/form-data' } })
            const newUrls = response.data.data.urls
            setFormData(prev => ({ ...prev, layoutImages: [...(prev.layoutImages || []), ...newUrls].slice(0, 5) }))
            toast.success(`${newUrls.length} layout image(s) uploaded successfully`)
        } catch (error: any) { toast.error(error.response?.data?.message || 'Failed to upload layout images') }
    }

    const removeFloorPlanImage = (blockId: string, imageUrl: string) => {
        const block = formData.blocks.find(b => b.blockId === blockId)
        if (block) updateBlock(blockId, 'floorPlanImages', block.floorPlanImages.filter(url => url !== imageUrl))
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.property === 'plots') {
            if (!formData.plots || formData.plots.length === 0) { toast.error('Please generate plots first'); return }
        } else {
            if (formData.blocks.length === 0) { toast.error('Please add at least one block'); return }
        }
        setLoading(true)
        const organization = getCookie('organization')
        const token = getCookie('token')
        if (!organization) { toast.error('Organization not found. Please login again.'); setLoading(false); return }
        try {
            const payload = { ...formData, organization }
            await axios.put(API.updateProject(decodedId), payload, { headers: { Authorization: `Bearer ${token}` } })
            toast.success('Project updated successfully!')
            navigate('/project_listing')
        } catch (error: any) {
            toast.error(error.response?.data?.message || error.response?.data?.errors?.[0]?.msg || 'Failed to update project')
                    ...
                    <div className='grid grid-cols-3 gap-4'>
                        <div className="grid col-span-2">
                                ...
                                <Card className="mb-4">
                                    <CardHeader className="pb-3">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <CardTitle className="flex items-center gap-2"><Layers className="h-5 w-5" />Plots Configuration</CardTitle>
                                    ...
                                    <CardContent>
                                        <div className="mb-6 p-4 border rounded-lg bg-muted/20">
                                            ...
                                            <div className="flex items-center gap-4">
                                                <Input type="file" accept="image/*" multiple className="max-w-xs" onChange={(e) => { handleLayoutImageUpload(e.target.files); e.target.value = '' }} />
                                                ...
                                                <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
                                                    {formData.layoutImages.map((url, idx) => (
                                                        <div key={idx} className="relative w-24 h-24 shrink-0 border rounded overflow-hidden group">
                                                            ...
                                                            <Button type="button" onClick={() => removeLayoutImage(url)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><X className="h-3 w-3" /></Button>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 p-4 bg-muted/30 rounded-lg">
                                            <div className="space-y-2"><Label>Total Plots</Label><Input type="number" min="1" max="1000" className="bg-background" value={plotsConfig.totalPlots} onChange={e => setPlotsConfig(prev => ({ ...prev, totalPlots: Math.min(1000, parseInt(e.target.value) || 1) }))} /></div>
                                            <div className="space-y-2"><Label>Numbering Pattern</Label>
                                                <Select value={formData.plotNumberPattern || 'numeric'} onValueChange={v => setFormData(prev => ({ ...prev, plotNumberPattern: v as 'numeric' | 'alpha' | 'custom' }))}>
                                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                                    <SelectContent><SelectItem value="numeric">Numeric (1, 2, 3...)</SelectItem><SelectItem value="alpha">Alphabetic (A, B, C...)</SelectItem></SelectContent>
                                                </Select>
                                            </div>
                                            <div className="space-y-2"><Label>Plot Size (sqft)</Label><Input type="number" min="1" className="bg-background" value={plotsConfig.defaultSize} onChange={e => setPlotsConfig(prev => ({ ...prev, defaultSize: parseInt(e.target.value) || 1000 }))} /></div>
                                            <div className="flex items-end"><Button type="button" onClick={generatePlots} className="w-full">{formData.plots && formData.plots.length > 0 ? 'Regenerate' : 'Generate'} Plots</Button></div>
                                        </div>
                                        {formData.plots && formData.plots.length > 0 ? (
                                            <div>
                                                <div className="flex items-center justify-between mb-3 p-3 bg-muted/30 rounded-lg">
                                                    <div className="flex items-center gap-4">
                                                        <div className="text-center"><p className="text-2xl font-bold">{formData.plots.length}</p><p className="text-xs text-muted-foreground">Total Plots</p></div>
                                                        <div className="text-center"><p className="text-lg font-semibold">{formData.plots.reduce((sum, p) => sum + p.size, 0).toLocaleString()}</p><p className="text-xs text-muted-foreground">Total Sqft</p></div>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <Button type="button" variant="outline" size="sm" disabled={plotPage === 0} onClick={() => setPlotPage(p => p - 1)}>Previous</Button>
                                                        <span className="text-sm px-2">Page {plotPage + 1} of {Math.ceil(formData.plots.length / PLOTS_PER_PAGE)}</span>
                                                        <Button type="button" variant="outline" size="sm" disabled={(plotPage + 1) * PLOTS_PER_PAGE >= formData.plots.length} onClick={() => setPlotPage(p => p + 1)}>Next</Button>
                                                    </div>
                                                </div>
                                                <div className="grid grid-cols-4 gap-3 px-3 py-2 bg-muted/50 rounded-t-lg text-sm font-medium"><div>Plot No.</div><div>Size (sqft)</div><div>Facing</div><div>Status</div></div>
                                                <div className="space-y-1 max-h-[70vh] overflow-auto">
                                                    {formData.plots.slice(plotPage * PLOTS_PER_PAGE, (plotPage + 1) * PLOTS_PER_PAGE).map((plot) => (
                                                        <div key={plot.plotId} className="grid grid-cols-4 gap-3 px-3 py-2 items-center border-b hover:bg-muted/20">
                                                            ...
                                                            <Input type="number" className="h-8" value={plot.size} onChange={e => updatePlot(plot.plotId, { size: parseInt(e.target.value) || 0 })} />
                                                            ...
                                                            <Select value={plot.status} onValueChange={v => updatePlot(plot.plotId, { status: v as 'available' | 'booked' | 'sold' })}>
                                                                <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
                                                                <SelectContent><SelectItem value="available">Available</SelectItem><SelectItem value="booked">Booked</SelectItem><SelectItem value="sold">Sold</SelectItem></SelectContent>
                                                            </Select>
                                                        </div>
                                                    ))}
                                                </div>
                                                <p className="text-xs text-muted-foreground text-center mt-2">Showing plots {plotPage * PLOTS_PER_PAGE + 1} - {Math.min((plotPage + 1) * PLOTS_PER_PAGE, formData.plots.length)} of {formData.plots.length}</p>
                                            </div>
                                        ) : (
                                            <div className="text-center py-8 text-muted-foreground border-2 border-dashed rounded-lg">
                                                <Layers className="h-12 w-12 mx-auto mb-2 opacity-50" /><p>No plots generated yet</p><p className="text-sm">Set total count, numbering pattern, and size, then click "Generate Plots"</p>
                                            </div>
                                        )}
                                    </CardContent>
                                </Card>
                            ...
                            {formData.property !== 'plots' && (
                                <Card className="mb-4">
                                    <CardHeader className="pb-3">
                                        <div className="flex items-center justify-between">
                                            <div><CardTitle className="flex items-center gap-2"><Layers className="h-5 w-5" />Blocks / Towers</CardTitle><CardDescription>Add blocks with floors and units</CardDescription></div>
                                            <Button type="button" onClick={addBlock} variant="outline" size="sm"><Plus className="h-4 w-4 mr-2" />Add Block</Button>
                                        </div>
                                    </CardHeader>
                                    <CardContent>
                                        {formData.blocks.length === 0 ? (
                                            <div className="text-center py-8 text-muted-foreground border-2 border-dashed rounded-lg"><Building2 className="h-12 w-12 mx-auto mb-2 opacity-50" /><p>No blocks added yet</p><p className="text-sm">Click "Add Block" to get started</p></div>
                                        ) : (
                                                        ...
                                                        <AccordionItem key={block.blockId} value={block.blockId} className="border rounded-lg mb-3 px-4">
                                                            <AccordionTrigger className="hover:no-underline">
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <span className="text-sm text-muted-foreground">({block.totalFloors} floors, {block.floors.reduce((acc, f) => acc + f.units.length, 0)} units)</span>
                                                                </div>
                                                            </AccordionTrigger>
                                                            <AccordionContent className="pt-4">
                                                                <div className="space-y-4">
                                                                                ...
                                                                                <ScrollArea className="h-20 mt-2">
                                                                                    <div className="flex gap-2">
                                                                                        {block.floorPlanImages.map((url, idx) => (
                                                                                            <div key={idx} className="relative group"><img src={url} alt={`Floor plan ${idx + 1}`} className="h-16 w-16 object-cover rounded border" />
                                                                                                <Button type="button" onClick={() => removeFloorPlanImage(block.blockId, url)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity"><X className="h-3 w-3" /></Button>
                                                                                            </div>
                                                                                        ))}
                                                                                    </div>
                                                                                </ScrollArea>
                                                                    ...
                                                                    <div className="flex justify-end mt-2"><Button type="button" variant="destructive" size="sm" onClick={() => removeBlock(block.blockId)}><Trash2 className="h-4 w-4 mr-2" />Remove Block</Button></div>
                                                                    <Separator />
                                                                    <div className="bg-muted/80 p-4 rounded-lg">
                                                                        <h4 className="font-medium mb-3 flex items-center gap-2"><DoorOpen className="h-4 w-4" />Generate Floors & Units</h4>
                                                                        <div className="grid grid-cols-3 gap-3 mb-4">
                                                                            <div className="space-y-1"><Label className="text-xs">Number of Floors</Label><Input type="number" className='bg-background dark:bg-[#09090b]' min="1" max="100" value={getBlockConfig(block.blockId).floors} onChange={e => updateBlockConfig(block.blockId, { floors: parseInt(e.target.value) || 10 })} /></div>
                                                                            <div className="space-y-1"><Label className="text-xs">Numbering</Label>
                                                                                <Select value={getBlockConfig(block.blockId).pattern} onValueChange={v => updateBlockConfig(block.blockId, { pattern: v })}>
                                                                                    <SelectTrigger className='bg-background dark:bg-[#09090b] w-full'><SelectValue /></SelectTrigger>
                                                                                    <SelectContent><SelectItem value="numeric">101, 102...</SelectItem><SelectItem value="alpha">1A, 1B...</SelectItem><SelectItem value="dash">1-1, 1-2...</SelectItem></SelectContent>
                                                                                </Select>
                                                                            </div>
                                                                            <div className="flex items-end"><Button type="button" variant="default" className="w-full mb-1" onClick={() => generateFloors(block.blockId)}>Generate Floors</Button></div>
                                                                        </div>
                                                                        <div className="border rounded-lg p-3 bg-background">
                                                                            <div className="flex items-center justify-between mb-2">
                                                                                <Label className="text-xs font-medium">Unit Types (per floor)</Label>
                                                                                <Button type="button" variant="outline" size="sm" onClick={() => addUnitType(block.blockId)}><Plus className="h-3 w-3 mr-1" />Add Type</Button>
                                                                            </div>
                                                                            ...
                                                                            <div className="mt-2 text-xs text-muted-foreground text-right">Total: {getBlockConfig(block.blockId).unitTypes.reduce((s, t) => s + t.count, 0)} units/floor</div>
                                                                        </div>
                                                                    </div>
                                                                    {block.floors.length > 0 && (
                                                                        <div className="mt-4">
                                                                            <div className="flex items-center justify-between mb-2"><h4 className="font-medium">Floors ({block.floors.length}) - Click to edit</h4><span className="text-xs text-muted-foreground">Each floor can have different units</span></div>
                                                                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2 max-h-[250px] overflow-auto p-1">
                                                                                {block.floors.map(floor => {
                                                                                    const bhkSummary = floor.units.reduce((acc, u) => { acc[u.bhk] = (acc[u.bhk] || 0) + 1; return acc }, {} as Record<number, number>)
                                                                                    const summaryText = Object.entries(bhkSummary).map(([bhk, count]) => `${count}×${bhk}BHK`).join(', ')
                                                                                    return (
                                                                                        <div key={floor.floorNumber} className="border rounded p-2 text-center text-sm cursor-pointer hover:bg-accent hover:border-primary transition-colors" onClick={() => openFloorEditor(block.blockId, floor.floorNumber)}>
                                                                                            <div className="font-medium flex items-center justify-center gap-1">{floor.floorName}<Edit2 className="h-3 w-3 opacity-50" /></div>
                                                                                            <div className="text-muted-foreground text-xs">{floor.units.length} units</div>
                                                                                            <div className="text-xs text-primary mt-1 truncate" title={summaryText}>{summaryText || 'No units'}</div>
                                                                                        </div>
                                                                                    )
                                                                                })}
                                                                            </div>
                        ...
                        <div className="grid col-span-1">
                            <Card className="h-fit mb-4">
                                <CardHeader className="pb-3">
                                    <CardTitle className="flex items-center gap-2"><Building2 className="h-5 w-5" />Project Details</CardTitle>
                                    <CardDescription>Basic information about the project</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="grid grid-cols-1 gap-4">
                                        <div className="space-y-2"><Label htmlFor="name">Project Name *</Label><Input id="name" className="bg-background dark:bg-[#09090b]" placeholder="Enter project name" value={formData.name} onChange={e => setFormData(prev => ({ ...prev, name: e.target.value }))} required /></div>
                                        <div className="space-y-2"><Label htmlFor="property">Property Type</Label>
                                            <Select value={formData.property} onValueChange={v => setFormData(prev => ({ ...prev, property: v }))}>
                                                <SelectTrigger className="w-full"><SelectValue placeholder="Select type" /></SelectTrigger>
                                                <SelectContent><SelectItem value="apartments">Apartments</SelectItem><SelectItem value="villas">Villas</SelectItem><SelectItem value="plots">Plots</SelectItem><SelectItem value="commercial">Commercial</SelectItem></SelectContent>
                                            </Select>
                                        </div>
                                ...
                                <CardFooter className="flex justify-end gap-3">
                                    <Button type="button" variant="outline" onClick={() => navigate('/project_listing')}><ArrowLeft className="h-4 w-4 mr-2" />Cancel</Button>
