...
// Using googleAdsService temporarly since we don't save anyway, or we can just mock the list to empty
import { MetaAdsStepper } from "@/components/integrations/meta-ads/meta_ads_stepper"
...
export default function MetaAdsIntegrationList() {
    ...
    useEffect(() => {
        setBreadcrumbs([
            ...
            { label: <TooltipProvider><Tooltip><TooltipTrigger><Info className="h-4.5 w-4.5 mt-1.5" /></TooltipTrigger><TooltipContent><p>Meta (Facebook/Instagram) Integration</p></TooltipContent></Tooltip></TooltipProvider> }
    ...
    const fetchIntegrations = async () => {
        setLoading(true)
        // Mocking return since it's just a UI page
        setTimeout(() => {
             setIntegrations([])
             setLoading(false)
        }, 600)
    ...
    const handleDelete = async (id: string) => {
        toast.error("Not implemented for Meta Ads")
    }
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-2">
                    <Badge variant="outline" className="bg-indigo-100/50 text-indigo-700 border-none px-2 rounded font-bold uppercase text-[10px]">
                        {row.original.source}
                    </Badge>
        ...
        <div className="flex flex-1 flex-col gap-6 px-8 py-6 max-w-[1400px] mx-auto w-full">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Meta Ads Integrations</h1>
                        <p className="text-muted-foreground text-sm">Manage your connected Facebook & Instagram lead forms</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    ...
                    <Button
                        ...
                        className="gap-2 font-semibold h-10 shadow-md transition-all hover:scale-[1.02] bg-indigo-600 hover:bg-indigo-700 text-white"
                    >
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
                            <div className="h-20 w-20 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-6">
                                <Facebook className="h-10 w-10 text-slate-400" />
                            </div>
                            <h3 className="text-xl font-bold mb-2">No integrations found</h3>
                            <p className="text-muted-foreground max-w-sm mb-8 leading-relaxed">
                                Connect your first Meta Lead form to start automatically syncing leads into your CRM.
                            </p>
                            <Button
                                onClick={() => setIsStepperOpen(true)}
                                className="gap-2 font-bold px-8 h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white"
                            >
            ...
            <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
                <DialogContent className="max-w-2xl p-0 overflow-hidden rounded-xl border-none shadow-2xl bg-white dark:bg-slate-950">
                    <div className="bg-indigo-900/10 dark:bg-muted/70 p-6 flex justify-between items-center border-b border-indigo-200 dark:border-white/10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-slate-900 dark:text-white text-xl font-bold">Meta Ads Integration</DialogTitle>
                        ...
                        <div className="bg-gray-50 dark:bg-muted/30 rounded-2xl border border-slate-200 dark:border-white/10 p-6 shadow-sm overflow-hidden relative">
                            ...
                            <div className="grid grid-cols-2 gap-y-8 gap-x-12">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <p className="text-base font-bold text-slate-900 dark:text-white">Meta Ads</p>
                                </div>
                                <div className="space-y-1.5">
                                ...
                                <div className="space-y-1.5 group cursor-pointer" onClick={() => copyToClipboard(viewingIntegration?.form_id)}>
                                    <div className="flex items-center gap-2 mb-1">
                                        <p className="text-[10px] uppercase font-bold tracking-widest text-slate-400">Meta Form ID</p>
                                ...
                                <div className="space-y-1.5 group cursor-pointer" onClick={() => copyToClipboard(viewingIntegration?.secret_key)}>
                                    ...
                                    <p className="text-xs font-mono text-indigo-600 dark:text-indigo-400 break-all bg-indigo-50/50 dark:bg-indigo-900/10 p-2 rounded-lg border border-indigo-100 dark:border-indigo-900/30">
