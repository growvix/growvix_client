...
export default function CpBulkUpload() {
    ...
    useEffect(() => {
        setBreadcrumbs([
            { label: "CP", href: "/cp" },
            { label: "Import Leads", href: "/cp/import_leads" },
            { label: "Bulk Upload" }
        ])
    }, [setBreadcrumbs])
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setTimeout(() => navigate("/cp/import_leads"), 1500)
        } catch (error: any) {
            ...
            <Card className="w-full max-w-4xl shadow-lg border-primary-200 pt-0 overflow-hidden">
                ...
                <CardContent className="p-0 bg-slate-50/50 dark:bg-neutral-950/20">
                    ...
                    <div className="flex justify-end px-8 pb-8">
                        <Button variant="ghost" onClick={() => navigate("/cp/dashboard")} className="rounded-full px-8 font-semibold">
                            Back to Dashboard
                        </Button>
                    </div>
                </CardContent>
