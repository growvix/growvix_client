...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        className="relative flex w-full flex-col gap-1 p-1 text-neutral-500 text-xs dark:text-neutral-400 font-extrabold"
        key={day}
      >
        {day}
        <div>
        ...
        {featuresForDay.length > 3 && (
          <span className="block text-neutral-500 text-xs dark:text-neutral-400">
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
  ...
  return (
    <div className={cn('grid flex-grow grid-cols-7', className)}>
      {daysData.map((day) => (
        <div className="p-3 text-center text-neutral-500 text-xs dark:text-neutral-400" key={day}>
          {day}
        </div>
      ))}
    </div>
